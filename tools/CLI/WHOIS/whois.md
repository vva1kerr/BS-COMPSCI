...
-q [version|sources|types]  query specified server infoWHOIS(1)                                                            Debian GNU/Linux                                                            WHOIS(1)
...
SYNOPSIS
       ...
       SOURCE[,SOURCE]... ] [ -T TYPE[,TYPE]... ] [ --verbose ] [ --no-recursion ] OBJECT
...
OPTIONS
       ...
       -i      ATTR[,ATTR]...  Inverse-search objects having associated attributes.  ATTR is the attribute name, while the positional OBJECT argument is
               the attribute value.
       ...
       -q      KEYWORD  Return information about the server.  KEYWORD can be version for the server version, sources for the list of database sources or
               types for the list of supported object types.
       ...
       -s      SOURCE[,SOURCE]...  Request the server to search for objects mirrored from SOURCE.  Sources are delimited by comma, and the order is sig‐
               nificant.  Use the -q sources parameter to obtain a list of valid sources.
